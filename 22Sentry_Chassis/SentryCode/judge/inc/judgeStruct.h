...
//注意：一定要加结构体的对齐，https://zhidao.baidu.com/question/418245481.html
...
typedef enum
{
	...
	//melancholycy add in 2022/3/8 to avoid C++ complier error 
	JUDGE_SPEED_TEST_ID = 0x4000,
	BREAK_FRAME_Num		= 0x0000
} frame_cmd_id_e;
//frame_cmd_id_e;
...
typedef  struct {	//字节对齐，https://blog.csdn.net/weibo1230123/article/details/84106028
...
}__attribute__((packed)) ext_student_interactive_all_data_t;
...
}__attribute__((packed)) ext_student_interactive_data_cmd_id_e;
...
typedef  struct
{
	...
	uint8_t layer; 
} __attribute__((packed)) __attribute__((packed)) ext_client_custom_graphic_delete_t;
//客户端删除图形
...
}__attribute__((packed)) self_control_interactive_all_data_t;
...
}__attribute__((packed)) ext_robot_command_t;//客户端下发
...
typedef  struct
{
	...
	float target_position_x;
	float target_position_y;
} __attribute__((packed)) ext_client_map_command_t;//客户端接收
...
typedef struct
{
	...
	uint16_t reserved;
}__attribute__((packed)) key_and_mouse_t;
